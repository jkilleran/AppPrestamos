...
class _DocumentsPageState extends State<DocumentsPage> {
  ...
  final String _apiBase = 'https://appprestamos-f5wz.onrender.com/api/document-status';
  ...
  void setDocumentStatusCode(int code) => setState(() => _status = decodeDocumentStatus(code));

  //==================== Utilidades ====================
  Future<String?> _getToken() async => (await SharedPreferences.getInstance()).getString('token');
  ...
  String _smtpSuggestion(String? raw) {
    ...
    if (lower.contains('smtp no configurado') || (lower.contains('smtp') && lower.contains('host') && lower.contains('no'))) {
  ...
  Future<void> fetchDocumentStatusFromBackend() async {
    ...
    try {
      final resp = await http.get(Uri.parse(_apiBase), headers: {'Authorization': 'Bearer $token'});
  ...
  Future<void> _pickAndSendDocument(DocumentType type) async {
    ...
    try {
      ...
      final uri = Uri.parse('https://appprestamos-f5wz.onrender.com/send-document-email');
      ...
      if (!kIsWeb && file.path != null) {
        ...
        req.files.add(http.MultipartFile.fromBytes('document', file.bytes!, filename: file.name));
      } else {
  ...
  Future<void> _adminFetchByEmail() async {
    ...
    if (email.isEmpty) return setState(() => _adminMessage = 'Ingrese un email');
    setState(() {
      _adminUpdating = true;
      _adminMessage = null;
    });
    ...
    try {
      final resp = await http.get(
        Uri.parse('https://appprestamos-f5wz.onrender.com/api/document-status/by-email?email=$email'),
  ...
  Future<void> _adminUpdateByEmail(int newCode) async {
    ...
    try {
      final resp = await http.put(
        Uri.parse('https://appprestamos-f5wz.onrender.com/api/document-status/by-email'),
        headers: {
  ...
  bool get _smtpConfigMissing {
    ...
    bool inTest = _testEmailMessage != null && _testEmailMessage!.toLowerCase().contains('smtp no configurado');
    bool inDump = _emailConfigDump != null && _emailConfigDump!.toLowerCase().contains('smtp no configurado');
          ...
          child: Column(
            ...
            children: [
              ...
              ElevatedButton.icon(
                ...
                onPressed: _sending[type]! ? null : () => _pickAndSendDocument(type),
              ),
              if (_sending[type]!)
                const Padding(
                ...
                Padding(
                  ...
                  child: Center(
                    child: Text(
                      _messages[type]!,
                      style: TextStyle(
                        color: _messages[type]!.contains('correctamente') ? Colors.green : Colors.red,
        ...
        children: [
          ...
          Wrap(
            ...
            children: [
              ...
              ElevatedButton(onPressed: _adminUpdating ? null : () => _adminUpdateByEmail(0), child: const Text('Set 0')),
              ElevatedButton(onPressed: _adminUpdating ? null : () => _adminUpdateByEmail(1), child: const Text('Set 1')),
              ElevatedButton(onPressed: _adminUpdating ? null : () => _adminUpdateByEmail(255), child: const Text('Set 255')),
            ],
          ),
          ...
          if (_targetEmailMessage != null)
            Padding(
              ...
              child: Text(
                _targetEmailMessage!,
                style: TextStyle(
                  color: _targetEmailMessage!.startsWith('Email destino actualizado') ? Colors.green : Colors.red,
          ...
          if (_fromEmailMessage != null)
            Padding(
              ...
              child: Text(
                _fromEmailMessage!,
                style: TextStyle(
                  color: _fromEmailMessage!.startsWith('Email remitente actualizado') ? Colors.green : Colors.red,
                  fontWeight: FontWeight.w600,
                ),
              ),
            ),
          ...
          if (_adminMessage != null)
            Padding(
              ...
              child: Text(_adminMessage!, style: const TextStyle(fontWeight: FontWeight.bold)),
            ),
  ...
  Widget _buildDiagnosticsSection() => Column(
        children: [
          const Divider(height: 32),
          const Text('Diagnóstico de correo', style: TextStyle(fontSize: 18, fontWeight: FontWeight.bold)),
          const SizedBox(height: 8),
          Wrap(
            spacing: 8,
            runSpacing: 8,
          ...
          if (_testEmailMessage != null)
            Padding(
              ...
              child: Text(
                _testEmailMessage!,
                style: TextStyle(
                  color: _testEmailMessage!.startsWith('Test') ? Colors.green : Colors.red,
                  fontWeight: FontWeight.w600,
                ),
              ),
            ),
